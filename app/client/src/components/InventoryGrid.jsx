        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden shadow-xl relative">
            ...
            <div ref={tableContainerRef} className="overflow-x-auto max-h-[calc(100vh-200px)] border-t border-zinc-700" style={{ scrollbarWidth: 'thin' }}>
                <table className="w-full text-sm text-left border-collapse table-fixed">
                    <thead className="text-xs text-white uppercase bg-slate-700 sticky top-0 z-30">
                        {/* 1) NEW ROW FOR COLUMN NUMBERS */}
                        <tr className="bg-slate-800 text-zinc-400 text-[10px] text-center">
                             <th className="border-r border-slate-600 bg-slate-800 sticky left-0 z-40 outline outline-1 outline-slate-600">
                        ...
                        <tr className="relative">
                            <th className="px-2 font-medium border-2 border-slate-400 bg-slate-700 w-[80px] min-w-[80px] max-w-[80px] relative group sticky left-0 z-40" style={{ height: `${headerHeight}px` }}>
                                Actions
                    ...
                    <tbody className="bg-slate-400">
                        {isAdding && (
                            <tr className="bg-yellow-100 border-2 border-yellow-500 shadow-inner">
                                <td className="px-4 py-3 border border-yellow-500 bg-yellow-50 sticky left-0 z-20 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.3)]">
                                    <div className="flex gap-2">
                        ...
                        {safeData.map((row, rowIndex) => {
                            ...
                            return (
                                <tr key={`row-${row._id}-${rowIndex}`} className={clsx("hover:bg-blue-400 transition-colors odd:bg-slate-300 even:bg-slate-400", isEditing && "!bg-blue-300")}>
                                    <td className="px-4 py-3 border border-slate-500 bg-inherit sticky left-0 z-20 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.3)]">
                                        <div className="flex gap-2">
                                            {isEditing ? (
                                                <>
