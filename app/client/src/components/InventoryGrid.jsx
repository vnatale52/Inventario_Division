            ...
            <div className="overflow-x-auto max-h-[calc(100vh-250px)] border-t border-zinc-700" style={{ scrollbarWidth: 'thin' }}>
                <table className="w-full text-sm text-left border-collapse table-fixed">
                    <thead className="text-xs text-zinc-400 uppercase bg-zinc-950/90 sticky top-0 z-10">
                        <tr>
                            <th className="px-4 py-3 font-medium border border-zinc-700 bg-zinc-900 sticky left-0 z-20 w-[100px] min-w-[100px]">
                                Actions
                            </th>
                            {safeColumns.map((col, index) => (
                                <th
                                    key={`col-${col.id}-${index}`}
                                    className="px-4 py-6 font-medium whitespace-nowrap border border-zinc-700 relative group"
                                    style={{ width: columnWidths[col.label] || getInitialWidth(col.label), minWidth: columnWidths[col.label] || getInitialWidth(col.label) }}
                                    title={col.label}
                                >
                                    <div className="flex flex-col gap-0.5 overflow-hidden">
                                        <span className="font-semibold text-zinc-200 text-xs truncate">{col.label}</span>
                                        {col.type && <span className="text-[10px] text-zinc-500 font-normal truncate">{col.type}</span>}
                                    </div>
                                </th>
                            ))}
                        </tr>
                            ...
                            return (
                                <tr key={`row-${row._id}-${rowIndex}`} className={clsx("hover:bg-zinc-800/50 transition-colors", isEditing && "bg-zinc-800/80")}>
                                    ...
                                    {safeColumns.map((col, colIndex) => {
                                        const val = getValue(row, col.label);
                                        return (
